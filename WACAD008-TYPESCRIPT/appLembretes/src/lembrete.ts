// Criação dos tipo usados Lembrete (elemento do array) lembretes (array de lembretes); editIndex (índice a ser editado)
type Lembrete = [string, Date, Date?, string?];

const lembretes: Array<Lembrete> = [];

let editIndex: number | null = null;

// Seleção dos elementos do DOM
const inputTitleReminder = document.getElementById("titleReminder") as HTMLInputElement | null;
const inputDeadlineReminder = document.getElementById("deadlineReminder") as HTMLInputElement | null;
const inputDescriptionReminder = document.getElementById("descriptionReminder") as HTMLInputElement | null;
const addReminderBtn = document.getElementById("addReminderBtn") as HTMLButtonElement | null;
...
function createOrUpdateReminder(titulo: string, prazo?: Date, descricao?: string): void {
  ...
  if (editIndex !== null) { // id do lembrete
    ...
    if (addReminderBtn) addReminderBtn.textContent = "Adicionar";// Reseta o texto do botão
  } else {
    // Criação
    const novoLembrete: Lembrete = [titulo, criadoEm, prazo, descricao]; // Cria um lembrete
...
function renderReminders(): void {
  ...
  lembretes.forEach(([titulo, criadoEm, prazo, descricao], index) => {
    ...
    editBtn.addEventListener("click", () => {                          
      ...
      if (inputTitleReminder) inputTitleReminder.value = titulo;                    
      if (inputDeadlineReminder) inputDeadlineReminder.value = prazo ? prazo.toISOString().slice(0, 16) : ""; 
      if (inputDescriptionReminder) inputDescriptionReminder.value = descricao ?? "";
      editIndex = index;                                                            // Guarda o índice do lembrete editado
      if (addReminderBtn) addReminderBtn.textContent = "Salvar Edição";
    });
...
function clearInputs(): void {
  if (inputTitleReminder) inputTitleReminder.value = "";
  if (inputDeadlineReminder) inputDeadlineReminder.value = "";
  if (inputDescriptionReminder) inputDescriptionReminder.value = "";
}
